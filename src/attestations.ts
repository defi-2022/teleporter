...
async function fetchAttestionsLocally(txHash: string): Promise<Attestation[]> {
  const response = await axios.get("/api/getAttestations", {
    params: {
      type: 'wormhole',
      index: txHash,
    },
...
export async function waitForAttestations(
  ...
  while (true) {
    ...
    const attestations = await fetchAttestionsLocally(txHash)
